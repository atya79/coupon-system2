...
//				answer = false;
   ...
   public static void namePasswordExist(String name, String password){
    	...
    	try {
			...
			PreparedStatement prep1 = DBConnector.getInstance().prepareStatement(sqlkNAME);
			prep1.setString(1, name);
			rs = prep1.executeQuery();
		
			String sqlPassword = "SELECT password FROM coupon.company WHERE `password`= ?" ;
			PreparedStatement prep2 = DBConnector.getInstance().prepareStatement(sqlPassword);
	   ...
	   try {
		   ...
		   DBConnector.getCon();
			String sqlName = "SELECT Comp_ID, password FROM company WHERE "
					...
					+ password + "'";
			stat = DBConnector.getInstance().createStatement();
